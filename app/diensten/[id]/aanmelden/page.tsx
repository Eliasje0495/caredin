...
import AanmeldenForm from "@/app/vacatures/[id]/aanmelden/AanmeldenForm";
...
export default async function DienstAanmeldenPage({ params }: { params: { id: string } }) {
  const session = await getServerSession(authOptions);
  if (!session) {
    redirect(`/inloggen?redirect=/diensten/${params.id}/aanmelden`);
  }
  ...
  if (!shift) notFound();
  if (existing) redirect(`/diensten/${params.id}?aangemeld=1`);
  if (shift.status !== "OPEN") redirect(`/diensten/${params.id}`);
  ...
  if (!profile) {
    ...
    if (shift.requiresBig) {
      ...
      requirements.push({
        label: "BIG-registratie", met,
        note: met ? "BIG-nummer geverifieerd"
          : profile.bigNumber ? "BIG-nummer ingediend, nog in afwachting van verificatie"
    ...
    if (shift.requiresSkj) {
      ...
      requirements.push({
        label: "SKJ-registratie", met,
        note: met ? "SKJ-registratie geverifieerd"
          : profile.skjNumber ? "SKJ-nummer ingediend, nog in afwachting van verificatie"
    ...
    if (shift.requiresKvk) {
      ...
      requirements.push({
        label: "KvK-inschrijving", met,
        note: met ? `KvK geverifieerd (${profile.kvkCompanyName ?? profile.kvkNumber})`
          : profile.kvkNumber ? "KvK-nummer ingediend, verificatie loopt"
    ...
    if (shift.minExperience > 0) {
      requirements.push({
        label: `Minimaal ${shift.minExperience} jaar werkervaring`, met: true,
  ...
  const backHref = `/diensten/${params.id}`;
  const successHref = `/diensten/${params.id}?aangemeld=1`;
    ...
    <>
      ...
      <div className="px-8 py-10 relative overflow-hidden" style={{ background: "var(--dark)" }}>
        ...
        <div className="max-w-3xl mx-auto">
          <Link href={backHref} className="no-underline text-sm font-medium"
