    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((i) => (
        <button key={i} type="button"
          ...
          style={{ fontSize: 26, color: i <= (hover || rating) ? "#F5A623" : "#D1D5DB", background: "none", border: "none", cursor: onSelect ? "pointer" : "default", padding: "0 2px" }}>
          ★
        </button>
      ))}
    </div>
...
export default function ReviewWorkerButton({ reviewedId, workerName, existingRating }: Props) {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)}
        className="text-xs font-semibold px-3 py-1.5 rounded-full mt-2"
        style={{ background: "var(--teal-light)", color: "var(--teal)", border: "none", cursor: "pointer" }}>
        Beoordeel professional →
      </button>

      {open && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,28,26,0.5)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
          onClick={(e) => e.target === e.currentTarget && setOpen(false)}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 36, width: "100%", maxWidth: 440 }}>
            <h2 style={{ fontFamily: "var(--font-fraunces)", fontSize: 20, fontWeight: 700, color: "var(--dark)", margin: "0 0 6px" }}>
              Hoe was {workerName}?
            </h2>
            <p style={{ fontSize: 13, color: "var(--muted)", margin: "0 0 20px" }}>Als professional bij jouw instelling</p>
            <div style={{ marginBottom: 20 }}><Stars rating={rating} onSelect={setRating} /></div>
